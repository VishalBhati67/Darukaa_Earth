...
def test_recorded_zero_biodiversity_is_kept():
    ...
    carbon, biodiversity = site_metric_snapshot(latest)
...
def test_dashboard_style_average_ignores_sites_without_readings():
    snapshots = [
        site_metric_snapshot(None),
        site_metric_snapshot(SimpleNamespace(carbon_value=10.0, biodiversity_score=80.0)),
        site_metric_snapshot(SimpleNamespace(carbon_value=5.0, biodiversity_score=60.0)),
    ]
    ...
    assert average_biodiversity([]) == 0.0
